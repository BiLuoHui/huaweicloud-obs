...
use Bihuohui\HuaweicloudObs\Signature\DefaultSignature;
...
trait Request
{
    public const INVALID_HTTP_METHOD_MSG = 'Method param must be specified, allowed values: GET | PUT | HEAD | POST | DELETE | OPTIONS';
    ...
    public function createPostSignature(array $args = []): Collection
    {
        $bucketName = !isset($args['Bucket']) ? null : strval($args['Bucket']);
        $objectKey = !isset($args['Key']) ? null : strval($args['Key']);
        $expires = !isset($args['Expires']) || !is_numeric($args['Expires']) ? 300 : intval($args['Expires']);
        ...
        if ($this->securityToken && !isset($formParams[Consts::SECURITY_TOKEN_HEAD])) {
            $formParams[Consts::SECURITY_TOKEN_HEAD] = $this->securityToken;
        }
        ...
        foreach ($formParams as $key => $val) {
            if ($key) {
                ...
                if (!in_array($key, Consts::ALLOWED_REQUEST_HTTP_HEADER_METADATA_NAMES)
                    && !str_starts_with($key, Consts::HEADER_PREFIX)
                    && !in_array($key, $conditionAllowKeys)
                ) {
                    $key = Consts::METADATA_PREFIX . $key;
                }
        ...
        $collection['OriginPolicy'] = $originPolicy;
        $collection['Policy'] = $policy;
        $collection['Signature'] = $signatureContent;
    ...
    public function __call($originMethod, $args)
    {
        ...
        if (isset($this->resource['aliases'][$method])) {
            $method = $this->resource['aliases'][$method];
        }

        $method = lcfirst($method);

        $operation = $this->resource['operations'][$method] ?? null;

        if (!$operation) {
            $obsException = new ObsException('unknown method ' . $originMethod);
        ...
        if (!$async) {
            ...
            $this->doRequest($collection, $operation, $params);
            unset($collection['method']);

            return $collection;
        } else {
            if (empty($args) || !(is_callable($callback = $args[count($args) - 1]))) {
                $obsException = new ObsException(
                    'async method ' . $originMethod . ' must pass a CallbackInterface as param',
                );
                $obsException->setType(ExceptionType::CLIENT);
                throw $obsException;
            }
            ...
            return $this->doRequestAsync($collection, $operation, $params, $callback, $start, $originMethod);
        }
    }

    private function checkMimeType($method, &$params): void
    {
        $uploadMethods = ['putObject', 'initiateMultipartUpload', 'uploadPart'];
        $hasContentTypeFlag = $this->hasContentType($params);

        if (in_array($method, $uploadMethods) && !$hasContentTypeFlag) {
            if (isset($params['Key'])) {
                $params['ContentType'] = MimeType::fromExtension(pathinfo($params['Key'], PATHINFO_EXTENSION));
            } elseif (isset($params['SourceFile'])) {
    ...
    private function hasContentType($params): bool
    {
    ...
    protected function makeRequest(
        ...
        $signatureInterface = new DefaultSignature(
            $this->ak,
            $this->sk,
            $this->pathStyle,
            $endpoint,
        ...
        $authResult = $signatureInterface->doAuth($operation, $params, $collection);
        $httpMethod = $authResult['method'];
        $authResult['headers']['User-Agent'] = 'HUAWEI_CLOUD_OBS_CLIENT_V1.0';
        if ($collection['method'] === 'putObject') {
            $collection['ObjectURL'] = ['value' => $authResult['requestUrl']];
        }
        return new \GuzzleHttp\Psr7\Request(
            $httpMethod,
            $authResult['requestUrl'],
            $authResult['headers'],
            $authResult['body'],
        );
    }

    protected function sendRequest(Collection $collection, $operation, $params, $request, $requestCount = 1): void
    {
        $saveAsStream = false;
        ...
        $promise = $this->httpClient->sendAsync($request, ['stream' => $saveAsStream])->then(
            function (Response $response) use (&$collection, $operation, $params, $request, $requestCount) {
                $statusCode = $response->getStatusCode();
                $readable = isset($params['Body']) && ($params['Body'] instanceof StreamInterface || is_resource(
                            $params['Body'],
                        ));
                ...
                if ($isRetryRequest && $location) {
                    ...
                    $port = $newUrl['port'] ?? $defaultPort;
                    $newEndpoint = $scheme . '://' . $newUrl['host'] . ':' . $port;
                    $this->doRequest($collection, $operation, $params, $newEndpoint);
                    return;
                }
                $this->parseResponse($collection, $request, $response, $operation);
            },

            function (RequestException $exception) use ($collection, $operation, $params, $request, $requestCount) {
                $this->parseException($collection, $request, $exception);
            },
        );
    ...
    protected function sendRequestAsync(
        ...
        return $this->httpClient->sendAsync($request, ['stream' => $saveAsStream])->then(
            ...
            function (RequestException $exception) use (
                $collection,
                $operation,
                $params,
                $callback,
                ...
                $obsException = $this->parseExceptionAsync($request, $exception);
                $callback($obsException, null);
            },
        );
    }

    public function createSignedUrl(array $args = []): Collection
    {
        if (!isset($args['Method'])) {
            $obsException = new ObsException(self::INVALID_HTTP_METHOD_MSG);
            $obsException->setType(ExceptionType::CLIENT);
            throw $obsException;
        }
        ...
        if ($this->securityToken && !isset($queryParams[Consts::SECURITY_TOKEN_HEAD])) {
            $queryParams[Consts::SECURITY_TOKEN_HEAD] = $this->securityToken;
        }
        ...
        $queryParams[Consts::TEMPURL_AK_HEAD] = $this->ak;
        ...
        $canonicalStr = $sign->makeCanonicalStr(
            $method,
            $headers,
            $queryParamsResult,
            $bucketName,
        ...
        $signatureContent = base64_encode(hash_hmac('sha1', $canonicalStr, $this->sk, true));

        $result .= 'Signature=' . $sign->urlencodeWithSafe($signatureContent);

        $collection = new Collection();
        $collection['ActualSignedRequestHeaders'] = $headers;
        $defaultPort = strtolower($url['scheme']) === 'https' ? '443' : '80';
        $port = $url['port'] ?? $defaultPort;
        $collection['SignedUrl'] = $url['scheme'] . '://' . $host . ':' . $port . $result;
